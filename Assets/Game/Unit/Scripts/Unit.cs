...
public class Unit : PhysicalEntity
{
    ...
    public IEnumerator RushToPosition(Node destinationNode)
    {
        ...
        while ((destination - pivot.position).magnitude > moveSpeed * Time.deltaTime * 2)
        {
            var pathPassedCoefficient = (destination - pivot.position).magnitude / (destination - startPos).magnitude;
            pivot.position += (destination - pivot.position).normalized * moveSpeed * (pathPassedCoefficient * 5f + 1f) * Time.deltaTime;
            yield return new WaitForSeconds(Time.deltaTime);
        }
        pivot.position = destination;
